        ...
        try:
            ...
            try:
                with self._stdoutInterceptor:
                    # Interactive REPL execution is intentional, but disabled in
                    # this QGIS plugin security build because scanners flag exec
                    # and opencratertool does not use the console widget.
                    raise RuntimeError(
                        "pyqtgraph console exec is disabled in the "
                        "opencratertool QGIS plugin build for security reasons"
                    )
