...
class ConsoleWidget(QtWidgets.QWidget):
    ...
    def loadHistory(self):
        ...
        if self.historyFile is not None and os.path.exists(self.historyFile):
            with open(self.historyFile, 'r', encoding='utf-8') as pf:
                data = json.load(pf)
    ...
    def saveHistory(self, history):
        ...
        if self.historyFile is not None:
            with open(self.historyFile, 'w', encoding='utf-8') as pf:
                json.dump(list(history), pf)
    ...
    def _stackItemDblClicked(self, handler, item):
        ...
        # External editor launch is disabled in this QGIS plugin security build.
        raise RuntimeError(
            "Opening an external editor from pyqtgraph.ConsoleWidget is "
            "disabled in the opencratertool QGIS plugin build for security reasons "
            f"(requested {fileName}:{lineNum})"
        )
